<?php
	...
	if ($_POST['TipoER']=="A") {
		require_once 'GrillaAnual.php';
        $orientationX="P";
	}
	
	if ($_POST['TipoER']=="B") {
		require_once 'GrillaBiAnual.php';
        $orientationX="P";
	}

	if ($_POST['TipoER']=="M") {
		require_once 'GrillaMensual.php';
        $orientationX="L";
	}
...
// exit;
    ...
    class MYPDF extends TCPDF {
        ...
        // Constructor
        public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false, $tfolio, $lfolio, $inicio, $headerText) {
            parent::__construct($orientation, $unit, $format, $unicode, $encoding, $diskcache, $pdfa);
            $this->textfolio = $tfolio;
            $this->largofolio = $lfolio;
        ...
        public function Header() {
            ...
            if($this->contador>0){
                $Ceros="";
                if($this->largofolio>0){
                    $largo=$this->largofolio-(strlen($this->contador));
                    $Ceros=str_repeat("0",$largo);
                }

                // Título
                $this->Cell(0, 15, $this->textfolio.''. $Ceros .''. $this->contador, 0, false, 'R', 0, '', 0, false);
    ...
    // Crea el objeto PDF
    // $pdf = new MYPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false, false, $TFOLIO, $CFOLIO, $Folio, $TMembrete);

    // Crea el objeto PDF
    $pdf = new MYPDF($orientationX, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false, false, $TFOLIO, $CFOLIO, $Folio, $TMembrete);
    // $pdf = new MYPDF('L', PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false, false, $Folio, $TMembrete);
...
// exit;
    ...
    // Genera el PDF
    $pdf->Output('EstadoResultadoV2_'.$_SESSION['RUTEMPRESA'].'_'.$_SESSION['PERIODO'].'.pdf', 'I');
?>