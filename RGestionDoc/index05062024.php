<?php
	...
	// $SwProceso=date('Ymd');
	// if($SwProceso<=20221206){
	// 	$mysqli=xconectar($_SESSION['UsuariaSV'],descript($_SESSION['PassSV']),$_SESSION['BaseSV']);

	// 	$SQL="SELECT COUNT(keyas) AS CONKEY, id, rutempresa, rut, periodo, ndoc, id_tipodocumento, keyas, monto, fecha, fregistro, tipo, origen, estado FROM `CTControRegDocPago` WHERE rutempresa='".$_SESSION['RUTEMPRESA']."' GROUP BY rut, rutempresa, periodo, ndoc, monto, id_tipodocumento";

	// 	$resultado = $mysqli->query("$SQL");
	// 	while ($registro = $resultado->fetch_assoc()) {
	// 		if($registro['CONKEY']>1){
	// 					$iId=$registro['id'];
	// 					// $registro['rutempresa'];
	// 					$iRut=$registro['rut'];
	// 					$iPer=$registro['periodo'];
	// 					$iNDo=$registro['ndoc'];
	// 					$iIdt=$registro['id_tipodocumento'];
	// 					$iKey=$registro['keyas'];
	// 					$iMon=$registro['monto'];
	// 					$iFec=$registro['fecha'];
	// 					$iFRe=$registro['fregistro'];
	// 					$iTip=$registro['tipo'];
	// 					$iOri=$registro['origen'];

	// 			$R="DELETE FROM CTControRegDocPago WHERE rutempresa='".$_SESSION['RUTEMPRESA']."' AND rut='$iRut' AND periodo='$iPer' AND ndoc='$iNDo' AND monto='$iMon' AND keyas='$iKey' AND id_tipodocumento='$iIdt' AND id>'$iId'";
	// 			// echo "<br>";
	// 			$mysqli->query("$R");
	// 		}
	// 	}
						...
						<p>Definir Cuenta</p>
							...
							<div class="col-md-6">
								<div class="input-group">
									...
									<div class="checkbox">
										<label><input type="checkbox" id="SwCC" name="SwCC" value="">Asignar este CC a todos los documentos de esta Raz&oacute;n Social</label>
									</div>
							</div>
						<div class="clearfix"></div>
					</div>
