...
// exit;
	...
	while ($Reg2 = $Resul2->fetch_assoc()) {
		...
		while ($registro = $resultados->fetch_assoc()) {
			...
			$InsSqlX="INSERT INTO CT14D VALUES('','$RutEmpresa','".$registro['periodo']."','1','','APERTURA','','".$registro['fecha']."','$sum','0','$sum','$sum','APERTURA','".$registro['keyas']."','','N','$BasTri','A');";
	...
	while ($registro = $resultados->fetch_assoc()) {
		...
		if ($row_cnt>0) {
			...
			while ($Reg = $Resul1->fetch_assoc()) {
				...
				$InsSql="INSERT INTO CT14D VALUES('','$RutEmpresa','".$registro['periodo']."','$Tipo','$LNume','$LTipo','$LRut','".$registro['fecha']."','$LNeto','$LIva','$LTotal','$D4','".$Glasa."','".$Reg['keyas']."','','N','$BasTri','A');";
				$mysqli->query($InsSql);
			}
		}else{
			...
			if ($registro['nfactura']=="PagBolEle") {
				...
				$InsSql="INSERT INTO CT14D VALUES('','".$_SESSION['RUTEMPRESA']."','".$registro['periodo']."','$Tipo','($LNume)','$TipoDTE-".$registro['nfactura']."','".$_SESSION['RUTEMPRESA']."','".$registro['fecha']."','$LNeto','$LIva','$LTotal','$LTotal','$Glasa','".$registro['keyas']."','','N','$BasTri','A');";
			}else{
				// $InsSql="INSERT INTO CT14D VALUES('','$RutEmpresa','$Periodo','$Tipo','','','','".$registro['fecha']."','$Monto','0','$Monto','$Monto','$Glasa','','N','$BasTri','A');";
				$InsSql="INSERT INTO CT14D VALUES('','$RutEmpresa','".$registro['periodo']."','$Tipo','','','','".$registro['fecha']."','$Monto','0','$Monto','$Monto','$Glasa','".$registro['keyas']."','','N','$BasTri','A');";
			}
			$mysqli->query($InsSql);
		}
	}
	...
	while ($Reg2 = $Resul2->fetch_assoc()) {
		...
		$NC = substr($Reg2["keyas"], 0, 2);


		$Xsql = "SELECT * FROM CTRegLibroDiario WHERE keyas='".$Reg2["keyas"]."' AND rutempresa='$RutEmpresa' AND glosa<>''";
		$Resul = $mysqli->query($Xsql);
		while ($Reg = $Resul->fetch_assoc()) {
			$Glasa=$Reg['glosa'];
		}
		...
		$row_cnt = $Resul->num_rows;
		if ($row_cnt==0) {
			$InsSql="INSERT INTO CT14D VALUES('','$RutEmpresa','".$Reg2['periodo']."','$t','$LNume','$LTipo','$LRut','".$Reg2['fecha']."','$LNeto','$LIva','$LTotal','0','$Glasa','".$Reg2['keyas']."','','N','S','X');";
			$mysqli->query($InsSql);
		}
	}
