<?php
	...
	function checkTimeout($start_time, $max_execution_time) {
		if (time() - $start_time > $max_execution_time) {
			...
			global $userrut;
			if (file_exists("cookiesRec".$userrut.".txt")) {
				unlink("cookiesRec".$userrut.".txt");
			}
    ...
    $cookie_file = "cookiesRec".$userrut.".txt";
    ...
    foreach ($loginArr as $clave => $valor) {
        ...
        if($i==13){
            ...
            if($VigHon=="N"){
                ...
                if ($row_cnt1==0) {
                    $Sql=$Sql."INSERT INTO CTHonorarios VALUES('','$Periodo','".$_SESSION['RUTEMPRESA']."','$FecHon','$RutHon','$NumHon','$xcuenta','$xccosto','$TotHon','$RetHon','$LiqHon','R','".date("Y-m-d")."','','A','');";
                    $ConDoc++;
                }
