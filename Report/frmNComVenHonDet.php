    ...
    if($_POST['id_tipodocumento'] == "H"){
        $query = "
            SELECT 
                ...
                cd.fecha AS fecha_documento,
                cd.liquido, 
                cd.tdocumento AS tipo_documento,
                cd.estado,
                cp.razonsocial,
                ld.fecha AS fecha_movimiento,
                ld.glosa AS observacion,
            ...
            JOIN 
                CTCliPro cp ON cd.rut = cp.rut
            LEFT JOIN
                CTRegLibroDiario ld ON cd.movimiento = ld.keyas
            WHERE 
                ld.glosa <>''
                AND cd.rutempresa = ? 
                AND cd.tdocumento <> ?
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        while ($RegPagos = $ResulPagos->fetch_assoc()) {
            ...
            while ($RegDiarios = $ResulDiarios->fetch_assoc()) {
                ...
                // if (!empty($Lfdesde) && !empty($Lfhasta) && $Lfdesde>=$RegPagos['fecha'] && $Lfhasta<=$RegPagos['fecha'] ) {
