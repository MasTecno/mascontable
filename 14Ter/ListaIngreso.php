...
?>
			...
			<dir class="col-md-12">
				...
				<div class="col-md-12 text-center">
					...
					<h3><?php 
						if ($_POST['SelPeriodo']=="C") {
							echo "Registros de Ingresos, A&ntilde;o ".$dano;
						}else{
							echo "Registros de Ingresos, ".$_POST['SelPeriodo'];
						}
						?></h3>
				</div>
				...
				<tbody>
					<?php
						...
						$mysqli=xconectar($_SESSION['UsuariaSV'],descriptSV($_SESSION['PassSV']),$_SESSION['BaseSV']);
						if ($_POST['SelPeriodo']=="C") {
							$StrSql="SELECT * FROM CTRegLibroDiario WHERE tipo='I' AND rutempresa='".$_SESSION['RUTEMPRESA']."' AND periodo like'%-$dano'";
						}else{
							$StrSql="SELECT * FROM CTRegLibroDiario WHERE tipo='I' AND rutempresa='".$_SESSION['RUTEMPRESA']."' AND periodo = '".$_POST['SelPeriodo']."'";
						}
						...
						while ($Reg = $Resul->fetch_assoc()) {
							...
							while ($Registros = $Resultado->fetch_assoc()) {
								...
								$Nivel=0;
								if ($TipDoc=="V") {
									$Sql1="SELECT * FROM CTRegDocumentos WHERE tipo='V' AND numero='$NumDoc' AND rut='$RutDoc' AND total='$MonDoc' AND rutempresa='".$_SESSION['RUTEMPRESA']."'";
									// echo "<br>";
								...
								if ($TipDoc=="V" && $Nivel==0) {
									// $Sql1="SELECT * FROM CTRegDocumentos WHERE tipo='V' AND numero='$NumDoc' AND rut='$RutDoc' AND total='$MonDoc' AND rutempresa='".$_SESSION['RUTEMPRESA']."'";
									$Sql1="SELECT * FROM CTRegDocumentos WHERE tipo='V' AND numero='$NumDoc' AND rut='$RutDoc' AND rutempresa='".$_SESSION['RUTEMPRESA']."'";
								...
								if ($Sw1==1) {
									...
									echo '
										<tr>
											...
											<td style="text-align: right;">'.$DifMont.'</td>
						...
						$D2=0;
						if ($_POST['SelPeriodo']=="C") {
							$StrSql="SELECT * FROM CTRegDocumentos WHERE tipo='V' AND rutempresa='".$_SESSION['RUTEMPRESA']."' AND periodo like'%-$dano'";
						}else{
							$StrSql="SELECT * FROM CTRegDocumentos WHERE tipo='V' AND rutempresa='".$_SESSION['RUTEMPRESA']."' AND periodo = '".$_POST['SelPeriodo']."'";
						}
