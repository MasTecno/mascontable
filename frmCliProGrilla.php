								...
								while ($registro = $resultados->fetch_assoc()) {
									...
									echo '
										<tr class="bg-white hover:bg-gray-50 transition duration-150 ease-in-out">
											<td class="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900" align="right">'.$registro["rut"].$SQLx1.'</td>
											...
											<td align="right">'.$NuCuenta.'</td>
											<td>'.$NCuenta.'</td>
									';

									echo '          <td><button type="button" class="btn btn-modificar btn-xs" onclick="Modifi('.$registro["id"].')">Modificar</button></td>';
									echo '          <td><button type="button" class="btn btn-cancelar btn-xs" onclick="Elimin('.$registro["id"].')">Eliminar</button></td>';
