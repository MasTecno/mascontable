...
// if (isset($_POST['SwProce']) && $_POST['SwProce']=="RRR" ) {
// 	// code...
// }
...
// exit;
	...
	while ($registro = $resultados->fetch_assoc()) {
		...
		while ($Reg = $Resu->fetch_assoc()) {
			if ($Reg['sdebe']!=$Reg['shaber']) {
				$Cadena=$Cadena."El comprobante N: ".$Xncomprobante.", de Fecha: ".$Xfecha.", Segun Glosa: ".$Xglosa."<br>";
			}
		}
	}
	...
	$Indece=0;
	$SQL="SELECT id, fecha, glosa, tipo, ncomprobante, keyas FROM CTRegLibroDiario WHERE rutempresa='$RutEmpresa' AND glosa<>'' AND periodo like '%-$Ano' ORDER BY fecha ASC";
	$resultados = $mysqli->query($SQL);
	while ($registro = $resultados->fetch_assoc()) {
		$Xkeyas=$registro["keyas"];
