				...
				try {
					...
					for ($i = 2; $i <= $filas; $i++) {
						...
						if ($_DATOS_EXCEL[$i]['Cuenta'] == "I" || $_DATOS_EXCEL[$i]['Cuenta'] == "E" || $_DATOS_EXCEL[$i]['Cuenta'] == "T") {
							...
							$STRInsert .= "INSERT INTO CTRegLibroDiario VALUES ('','$Per','$RutEmpresa','$Fec','" . strtoupper($_DATOS_EXCEL[$i]['Glosa']) . "','0','0','0','" . date("Y-m-d") . "','A','$KeyAs','','','$Folio','" . $_DATOS_EXCEL[$i]['Cuenta'] . "','0','','0','','0','0');";
