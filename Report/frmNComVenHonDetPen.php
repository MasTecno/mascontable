...
// exit;
	...
	if ($_POST['id_tipodocumento']=="H") {
		// $SQL="SELECT rut FROM CTHonorarios WHERE rutempresa='$RutEmpresa' AND tdocumento='R' GROUP BY rut";
	...
	$Indece=0;
	$Str = "SELECT * FROM CTCliPro WHERE tipo='$tCliPro'";
	...
	// echo $Str=$Str." ORDER BY razonsocial";
	...
	while ($reg = $resul->fetch_assoc()) {
		...
		// print_r($_SESSION['ListRut']);

		// print_r($_SESSION['ArrayRut']);
		...
		foreach($_SESSION['ArrayRut'] as $indice=>$ArrayRegRut){
			...
			if ($_POST['id_tipodocumento']=="H") {
				...
				$SQL="SELECT * FROM CTHonorarios WHERE rutempresa='$RutEmpresa'";
				...
				$SQL="SELECT * FROM CTRegDocumentos WHERE rutempresa='$RutEmpresa' AND tipo='$tidoc'";
				$SQL=$SQL." AND rut='".$ArrayRegRut['Rut']."'";
			...
			while ($registro = $resultados->fetch_assoc()) {
				...
				$SQL1="SELECT IFNULL(sum(monto), 0) AS LMonto FROM CTControRegDocPago WHERE rutempresa='$RutEmpresa' AND id_tipodocumento='$XTipDocumento' AND rut='".$ArrayRegRut['Rut']."' AND tipo='$tidoc' AND ndoc='$NDocu'";
				if ($_POST['fdesde']!="" || $_POST['fhasta']!="") {
				...
				while ($registro1 = $resultados1->fetch_assoc()) {
					...
					if ($MontoReg>$registro1['LMonto']) { 
						$SwSi="SI";
						$KeyDiario=$registro1['keyas'];
					}
				}
				...
				if ($SwSi=="SI") {
					...
					if ($ValKeyas!="") {
						$SQL1="SELECT * FROM CTRegLibroDiario WHERE keyas='$ValKeyas' AND glosa<>'' AND rutempresa='".$_SESSION['RUTEMPRESA']."'";
						$resultados1 = $mysqli->query($SQL1);
						while ($registro1 = $resultados1->fetch_assoc()) {
					...
					$SQL1="SELECT * FROM CTControRegDocPago WHERE rutempresa='$RutEmpresa' AND id_tipodocumento='$XTipDocumento' AND rut='".$ArrayRegRut['Rut']."' AND tipo='$tidoc' AND ndoc='$NDocu'";
					if ($_POST['fdesde']!="" || $_POST['fhasta']!="") {
						$SQL1=$SQL1." AND fecha BETWEEN '$Lfdesde' AND '$Lfhasta'";
					}
					...
					while ($registro1 = $resultados1->fetch_assoc()) {
						...
						$SQL2="SELECT * FROM CTRegLibroDiario WHERE keyas='".$registro1["keyas"]."' AND glosa<>'' AND rutempresa='$RutEmpresa'";
