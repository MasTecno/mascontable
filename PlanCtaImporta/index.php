		...
		if (isset($action)== "upload" && $_FILES['excel']['type']=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"){
			...
			if (file_exists ("Temp_".$archivo)){
				...
				for ($i=2;$i<=$filas;$i++){
					...
					if($_DATOS_EXCEL[$i]['Codigo']==""){
						break;
					}

					$idCat="";
					// echo "SELECT * FROM CTCategoria WHERE nombre='".$_DATOS_EXCEL[$i]['Categoria']."' AND estado='A'<br>";
					// exit;
					$resultado = $mysqli->query("SELECT * FROM CTCategoria WHERE nombre='".$_DATOS_EXCEL[$i]['Categoria']."' AND estado='A'");
					...
					if($EnEmpresa=="N"){
						$STRInseert=$STRInseert."INSERT INTO CTCuentas VALUES('','".$_DATOS_EXCEL[$i]['Codigo']."','".($_DATOS_EXCEL[$i]['Nombre'])."','".$idCat."','N','N','A'); ";
					}else{
						$STRInseert=$STRInseert."INSERT INTO CTCuentasEmpresa VALUES('','$RutEmpresa','".$_DATOS_EXCEL[$i]['Codigo']."','".($_DATOS_EXCEL[$i]['Nombre'])."','".$idCat."','N','N','A'); ";
					}
				}
				...
				if ($Msj=="") {
					...
					if($EnEmpresa=="N"){
						$mysqli->query("DELETE FROM CTCuentas");
					}else{
		...
		<form name="form1" method="post" action="" enctype="multipart/form-data">
			...
			<div class="col-sm-4">
				<?php
					...
					if($EnEmpresa=="N"){
						$SQL="SELECT * FROM CTRegLibroDiario";
					}else{
