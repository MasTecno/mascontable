<?php
    ...
    function login($urut,$upass,$mper,$aper) {
        try {
            ...
            function obtenerDatosPagina($ch, $pagina, $mper, $aper) {
            ...
            //* Primera pagina
            $result = obtenerDatosPagina($ch, $pagina, $mper, $aper);
            if (!$result) exit(0);
            ...
            $info = $dom->getElementsByTagName("td");
            foreach ($info as $td) {

                $texto = trim($td->nodeValue);

                if (preg_match('/Página\s+\d+\s+de\s+(\d+)/', $texto, $matches)) {
                    $totalPaginas = (int)$matches[1];
                    break;
                }
                
            }

            // echo "Paginas encontradas: $totalPaginas" . "<br>";
            
            //* Boletas pagina 1
            $boletasPaginaUno = procesarDatosPagina($dom);
            $todasBoletas = array_merge($todasBoletas, $boletasPaginaUno);
            

            //* Obtener datos de las paginas restantes
            for ($i = 2; $i <= $totalPaginas; $i++) {
                
                // echo "Pagina $i de $totalPaginas <br>";
                
                $result = obtenerDatosPagina($ch, $i, $mper, $aper);

                $dom = new DOMDocument("1.0", "UTF-8");
                $htmlUtf8 = "<?xml encoding='UTF-8'>" . $result;
                $dom->loadHTML($htmlUtf8, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);

                //* Procesar cada pagina para obtener los registros
                $boletasPagina = procesarDatosPagina($dom);
                $todasBoletas = array_merge($todasBoletas, $boletasPagina);

            }

            // echo "<pre>";
            // print_r($todasBoletas);
            // echo "</pre>";
            // exit;

            // if(count($todasBoletas) === 0) {
            //     echo "No hay boletas";
            //     exit;
            // }
    ...
    function procesarDatosPagina($dom) {
        $informe = [];

        $tables = $dom->getElementsByTagName("table");

        //* Procesar tabla de boletas
        if ($tables->length > 3) {
