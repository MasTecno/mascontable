...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function Procesar(){
				if (form1.cacuenta.value!="") {
					var url= "frmNCuentasDet.php";
					$.ajax({
						type: "POST",
						url: url,
					...
					<select class="form-control" id="cacuenta" name="cacuenta">
						...
						<?php
							...
							if ($_SESSION["PLAN"]=="S"){
								...
								$SQL="SELECT * FROM CTCuentas ORDER BY detalle";
							}
							$resultados = $mysqli->query($SQL);
							while ($registro = $resultados->fetch_assoc()) {

								$SQL1="SELECT cuenta FROM CTRegLibroDiario WHERE cuenta='".$registro['numero']."' AND rutempresa='$RutEmpresa' AND cuenta<>'0' GROUP BY cuenta";
								$resultados1 = $mysqli->query($SQL1);
								while ($registro1 = $resultados1->fetch_assoc()) {
									echo '<option value="'.$registro['id'].'">'.$registro['numero'].' - '.$registro['detalle'].'</option>';
								}
							}
							$mysqli->close();
						?>
